...
class AutoPlayer(Player):
    ...
    def choose(self, choices):
        mychoice = random.choice(self.hand)
        while(not self.followsRules(mychoice, choices)):
            mychoice = random.choice(self.hand)
        self.hand.remove(mychoice)
        time.sleep(2)
        return mychoice    
...
class HumanPlayer(Player):
    ...
    def choose(self, choices):
        ...
        while(True):
            ...
            for card in self.hand:
                if card.suite == input_suite and card.number == input_number and self.followsRules(card,choices):
